...
Запустить линтеры: `go vet` и проверку документации экспортированных API:
...
Собрать клиент для Windows, Linux и macOS:
...
- `shift+tab` на экране входа переключает login/register;
- `shift+tab` на structured-формах переключает credentials/card;
- `shift+tab` на blob-форме переключает text/binary;
...
- `d` открывает подтверждение удаления выбранного секрета;
- `l` открывает подтверждение logout и удаления локальных `session.json`/`secrets.json`;
- `s` на экране blob-секрета скачивает файл;
- `p` на экране blob-секрета заменяет файл;
- `y` подтверждает delete/logout, `n` отменяет;
- `esc` возвращает к списку после входа, на экране входа игнорируется;
- `shift+q` выходит из списка или возвращает из просмотра к списку.

В TUI поле пользовательской метаинформации называется `description`. Можно вводить обычный текст, например `рабочий github`; клиент сохранит его в серверное поле `metadata` как `{"description":"рабочий github"}`. Для расширенного сценария можно ввести JSON object, например `{"site":"github.com","owner":"work"}`.
