...
func (m *useCaseMock) Execute(_ context.Context, userID uuid.UUID, secretID uuid.UUID) error {
	m.userID = userID
	m.secretID = secretID
	return m.err
}
...
func TestHandler(t *testing.T) {
	...
	t.Run(
		"Должен вернуть 404", func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest(http.MethodDelete, "/api/v1/secrets/"+uuid.New().String(), nil)
			request.Header.Set("Authorization", "Bearer token")
...
func newTestRouter(t *testing.T, useCase UseCase, userID uuid.UUID) http.Handler {
	...
	router.With(middleware.Auth(&tokenValidatorMock{claims: token.Claims{UserID: userID, Login: "igor"}})).
		Delete("/api/v1/secrets/{id}", handler.ServeHTTP)
	return router
}
